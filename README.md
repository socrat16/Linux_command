...
ФС- метод хранения файлы на диске (fat,ext,ntfs)
Принцип хранения: 
ФС содержат inode
...
С дисками как с файлами работаем благодаря VFS, монитрование это связка файла блоч ус-ва к каталогу. (мы работаем с VFS, а за ней связка ОС - драйвера реал. ФС )
`/etc/fstab` - подмантированные ФС про загрузке ОС
`lsblk` - инфо о блочных ус-вах
`mount mount /dev/vda /mnt/`  - что куда подмонтрировано
